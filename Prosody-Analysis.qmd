...
A **log-normal distribution** was assumed for the model family, meaning that F0 is modeled as following a lognormal distribution. This is equivalent to assuming that log(F0) follows a normal distribution. During model fitting, coefficient values were identified that maximized the probability of the observed data under the assumed model. The model estimates effects on the log scale internally, but predictions are automatically transformed to the original Hz scale.
...
```{r posterior-draws}
# Extract posterior draws and calculate predictions manually from coefficients
f0_bm_draws <- as_draws_df(f0_bm) %>% 
  mutate(
    f_neutral = exp(b_Intercept),
    f_disgust = exp(b_Intercept + b_EmotionDisgust),
    f_fear = exp(b_Intercept + b_EmotionFear),
    f_anger = exp(b_Intercept + b_EmotionAnger),
    f_happy = exp(b_Intercept + b_EmotionHappy),
    f_sad = exp(b_Intercept + b_EmotionSad),
    m_neutral = exp(b_Intercept + b_SexMale),
    m_disgust = exp(b_Intercept + b_SexMale + b_EmotionDisgust + `b_EmotionDisgust:SexMale`),
    m_fear = exp(b_Intercept + b_SexMale + b_EmotionFear + `b_EmotionFear:SexMale`),
    m_anger = exp(b_Intercept + b_SexMale + b_EmotionAnger + `b_EmotionAnger:SexMale`),
    m_happy = exp(b_Intercept + b_SexMale + b_EmotionHappy + `b_EmotionHappy:SexMale`),
    m_sad = exp(b_Intercept + b_SexMale + b_EmotionSad + `b_EmotionSad:SexMale`)
  ) %>%
  select(f_anger, f_disgust, f_fear, f_neutral, f_happy, f_sad, 
         m_anger, m_disgust, m_fear, m_neutral, m_happy, m_sad)

# Convert to long format for analysis
draws_long <- f0_bm_draws %>%
  ...
  separate(condition, into = c("Sex", "Emotion"), sep = "_") %>%
  mutate(
    Sex = if_else(Sex == "f", "Female", "Male"),
    Emotion = str_to_title(Emotion)
  )

# Calculate condition means with credible intervals
f0_summary <- draws_long %>%
  group_by(Sex, Emotion) %>%
  summarise(
    Estimate = mean(F0),
    Est.Error = sd(F0),
    Q5 = quantile(F0, 0.025),
    Q10 = quantile(F0, 0.10),
    Q90 = quantile(F0, 0.90),
    Q95 = quantile(F0, 0.975),
    .groups = "drop"
  ) %>%
  arrange(Sex, Emotion)
```
...
diff_vs_neutral <- draws_long %>%
  ...
  summarise(
    ...
    q5 = quantile(diff_vs_neutral, 0.025),
    q95 = quantile(diff_vs_neutral, 0.975),
    .groups = "drop"
  ) %>%
  arrange(Sex, Emotion)
```
...
sex_diffs <- draws_long %>%
  ...
  summarise(
    ...
    q5 = quantile(diff_M_minus_F, 0.025),
    q95 = quantile(diff_M_minus_F, 0.975),
    .groups = "drop"
  ) %>%
  arrange(Emotion)
```
